...
import { QUERY_TOP_LVL_HOLDER_INFO } from '../utils/queries';
...
const TopLVLHolder: FC = () => {
  const info = useQuery(QUERY_TOP_LVL_HOLDER_INFO());
    ...
    <div className="max-w-1170px lg-my-30px mx-auto">
      <div className="display-none lg-flex items-center justify-between">
        <div className="text-#000 px-18px font-bold lg-text-20px text-16px trapezoid-right relative">
          <div className="my-5px font-800">Top LVL Holders</div>
        </div>
      </div>
      <table className="w-100% border-separate border-spacing-y-6px border-spacing-x-0 display-none lg-display-table b-t-1px b-t-solid b-t-#53FF8A text-14px">
        <thead className="color-#ADABAB text-12px">
          <tr>
            ...
            <th className="font-400 py-20px pb-6px px-12px text-left w-15%">
              <span className="ic-sort relative pr-15px">Total</span>
            </th>
            <th className="font-400 py-20px pb-6px px-12px text-left w-10%">In Wallet</th>
            <th className="font-400 py-20px pb-6px px-12px text-left w-15%">In DAO Staking</th>
            <th className="font-400 py-20px pb-6px px-12px text-left w-15%">In LVL Staking</th>
          </tr>
        </thead>
        <tbody>
          {info?.data?.map((row, i) => (
      ...
      <div className="display-block lg-display-none px-16px">
        <div className="font-bold mb-16px text-14px">Top LVL Holders</div>
        {info?.data?.map((row, i) => (
